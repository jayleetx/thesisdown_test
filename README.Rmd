...
- Double check that the files referenced for `bibliography` and `csl` are correct. If you change the citation format or have a differently named file, 

- `lot` and `lof` are "List of tables" and "List of figures" respectively, leave this as true to match the thesis formatting requirements.
...
By default, [`_bookdown.yml`](index/_bookdown.yml) handles the chapter ordering. The third line of this file (`rmd_files`) defines which files are combined to create the actual thesis document. You can add a chapter by adding it to the directory and this list, and delete one by removing it from the list.
